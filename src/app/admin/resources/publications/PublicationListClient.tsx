...
export default function PublicationListClient({ userId }: PublicationListClientProps) {
  ...
  const fetchPublications = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/cms/publications?${params}`);
  ...
  if (loading && publications.length === 0) {
    return <DataLoader size="lg" text="Loading publications..." />;
  }
    ...
    <div className="space-y-6">
      ...
      {publications.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-12 text-center">
          <p className="text-gray-500 dark:text-gray-400 mb-4">No publications found</p>
          <Link
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {publications.map((publication) => (
                  <tr key={publication.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          href={`/admin/resources/publications/${publication.id}`}
