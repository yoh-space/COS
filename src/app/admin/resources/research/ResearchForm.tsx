  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const url = mode === 'edit' && initialData
        ? `/api/cms/research/${initialData.id}`
        : '/api/cms/research';
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || 'Failed to save research activity');
      }

      router.push('/admin/resources/research');
    } catch (error) {
      alert(error instanceof Error ? error.message : 'Failed to save research activity');
      setLoading(false);
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex gap-4">
        ...
        >
          {loading ? 'Saving...' : mode === 'edit' ? 'Update Research Activity' : 'Create Research Activity'}
        </button>

        <button
          type="button"
          onClick={() => router.push('/admin/resources/research')}
          disabled={loading}
